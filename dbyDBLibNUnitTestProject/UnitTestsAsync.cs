...
namespace SQLServerClientLibraryNUnitTestProject
{
	public class UnitTestsAsync
	{
		...
		/// <summary>
		/// See what happens if we call getdate scalar function asynchronously
		/// </summary>
		/// <returns></returns>
		[Test]
		...
		/// </summary>
		[Test]
		public async Task StoredProcedureDoNothingTest()
		{
			...
			Tuple<int?, DataSet> result = await mycaller.StoredProcedureAsync(donothing);
		...
		[Test]
		public async Task StoredProcedureReturnNumberTest()
		{
			...
			Tuple<int?, DataSet> result = await mycaller.StoredProcedureAsync(returnnumber, parameterlist);
			Assert.IsNotNull(result);
		...
		/// </summary>
		[Test]
		public async Task StoredProcedureGetAllDataTypesTest()
		{
			...
			Tuple<int?, DataSet> result = await mycaller.StoredProcedureAsync(selectall);
			Assert.IsNotNull(result);
		...
		/// <summary>
		/// All this test does is check that we can successfully call an async method
		/// </summary>
		[Test]
		public async Task StoredProcedureWaitForSecondsTest()
		{
			...
			//	The stored procedure is expected to return a dataset containing one table which has a single row
			Tuple<int?, DataSet> result = await mycaller.StoredProcedureAsync(waitfortest, parameterlist);
		...
		public void StoredProcedureThrowDivideByZeroErrorTest()
		{
			...
			var ex = NUnit.Framework.Assert.ThrowsAsync<SqlException>(async () => await mycaller.StoredProcedureAsync(divbyzerotest));
			Assert.AreEqual("Divide by zero error encountered.", ex.Message);
		}
	}
}
